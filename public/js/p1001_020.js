...
// project: p1001_020,  Spanish version, 55min & 24hr delays
...
// study lists with 1 recalls for all pairs, and only 12 pairs
/*
var mL1 = [
    ['waist','spray',0,0,1],
    ['stove','dirt',0,0,1],
...
*/
var del3L1 = [
    ['cable','tribu', 'fuego', 'isla', 'calle','option3','option','resp1',0, null],
    ['lluvia','cuadro', 'freno', 'cama', 'tribu','option4','option','resp1',0, null],
    ['cuento','norte', 'planta', 'fuga', 'agua','option2','option','resp1',0, null],
    ['cinta','puente', 'pila', 'pacto', 'mano','option1','option','resp1',0, null],
    ['tela','isla', 'mancha', 'banco', 'carta','option3','option','resp1',0, null],
    ['copa','foto', 'puerto', 'puente', 'fuga','option4','option','resp1',0, null],
    ['humo','boca', 'noche', 'disco', 'cuello','option2','option','resp1',0, null],
    ['tierra','cuadro', 'miga', 'seda', 'ruido','option1','option','resp1',0, null],
    ['coche','pasta', 'banco', 'gancho', 'pelo','option1','option','resp1',0, null],
    ['sierra','planta', 'siglo', 'brazo', 'disco','option4','option','resp1',0, null],
    ['torre','pasta', 'choque', 'mano', 'viaje','option3','option','resp1',0, null],
    ['falda','ruido', 'premio', 'pueblo', 'noche','option1','option','resp1',0, null]
];
...
var dL1 = [['bomba','zumo',0,0,2,0], ['globo','tropa',0,0,2,0],['carne','silla',0,0,2,0]];
...
function test() {
	...
	*/
	...
	// setup for 24hr clock
	var ampm = "";

	/*
	// setup for 12hr clock
	var ampm = "am";
	if( hr > 12 ) {
	    hr -= 12;
	    ampm = "pm";
	...
	*/
	...
	// calculate 24hr test target time and using 24hr clock
	n = new Date(endTime + hour24);
	hr = n.getHours();
	min = (n.getMinutes() < 10 ? '0' : '') + n.getMinutes();
	ampm = "";
	var dateDay = n.getDate();
	var monthList = ["Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Deciembre"];
	var month = monthList[n.getMonth()];
	var year = n.getFullYear();

	var h24 = hr + ":" + min + ampm + " " + dateDay + " " + month + " " + year;
...
function checkRecognitionAnswer() {
    ...
    if (select_pos == "none") {
	alert( "Uste debe seleccionar una respuesta. Si no está seguro, por favor, adivine." );
    }
